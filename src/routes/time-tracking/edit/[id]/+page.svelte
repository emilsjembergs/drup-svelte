<script>
  ...
  const timeEntryId = $page.params.id;
  ...
  async function loadTimeEntry(id) {
    try {
      const response = await fetch(`http://localhost:3000/api/time-entries/${id}`, {
      ...
      // Update form data with time entry details
      formData = {
        ...formData,
        id: timeEntry.id,
        user_id: timeEntry.user_id,
        project_id: String(timeEntry.project_id),
        date: timeEntry.date.slice(0, 10), // YYYY-MM-DD format
        hours: timeEntry.hours,
        description: timeEntry.description || '',
        entry_type: timeEntry.entry_type || 'work'
      };
      
      originalProjectId = timeEntry.project_id;
      
      // Load project users and funding
      if (timeEntry.project_id) {
        if (['admin', 'project_manager', 'hr'].includes($currentUser?.role)) {
          await loadProjectUsers(timeEntry.project_id);
        }
        await loadProjectFunding(timeEntry.project_id);
      }
      
      // Load funding distribution if available
      if (timeEntry.funding_distribution && timeEntry.funding_distribution.length > 0) {
        useCustomDistribution = true;
        customDistributions = timeEntry.funding_distribution.map(item => ({
          funding_source_id: String(item.funding_source_id),
          percentage: item.percentage,
          name: item.name || fundingSources.find(s => s.id === item.funding_source_id)?.name
        }));
        recalculatePercentages();
      }
    } catch (err) {
      console.error('Error loading time entry:', err);
      error = 'Failed to load time entry. ' + (typeof err === 'string' ? err : err.message || '');
    }
  }
  ...
  async function loadProjectFunding(projectId) {
    ...
    try {
      ...
      if (projectFunding && projectFunding.length > 0) {
        ...
        // Set custom distributions from project funding if not already set
        if (!useCustomDistribution || formData.project_id !== originalProjectId) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      console.log("Updating time entry:", timeEntry);
      
      // Send request to update time entry
      const response = await fetch(`http://localhost:3000/api/time-entries/${timeEntryId}`, {
        method: 'PUT',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json'
      ...
      error = typeof err === 'string' ? err : err.message || 'Failed to update time entry';
...
<div class="container mx-auto px-4 py-8">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Rediģēt Darba Laika Ierakstu</h1>
    <button
